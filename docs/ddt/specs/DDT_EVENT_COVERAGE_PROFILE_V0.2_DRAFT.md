# DDT Event Coverage Profile v0.2 — Draft

**Project:** Diamond Data Chain (DDC)  
**Specification ID:** `DDT-EVENT-COVERAGE-PROFILE-0.2-DRAFT.1`  
**Version:** `0.2.0-draft.1`  
**Status:** DRAFT — not approved for production conformance or completeness claims  
**Date:** 2026-09-05  
**Parent standard:** `DDC-TRS-2.0-DRAFT.1`  
**Time/checkpoint dependency:** `DDT-TIME-CHECKPOINT-PROFILE-0.2-DRAFT.1`
...
- `schemas/ddt-event-coverage-contract-v0.2.schema.json`;
- `schemas/ddt-event-coverage-evidence-v0.2.schema.json`; and
- `schemas/ddt-event-coverage-profile-v0.2.schema.json`; and
- `profiles/ddt-closed-event-coverage-profile-v0.2.json`.
...
### 3.1 v0.2 successor boundary

Version 0.2 preserves the v0.1 source-position, tombstone-core, coverage-leaf, event-set-tree, statement-core, witness-core and scoped-completeness semantics. It changes the three signature-bearing coverage carriers from detached `proofRefs` to complete common Signature Proof v0.2 objects. Historical v0.1 carriers remain verifiable under their original contracts and MUST NOT be reinterpreted as v0.2 carriers.

The exact allowed pairs are:

- Coverage Tombstone: `COVERAGE_TOMBSTONE_ATTESTATION` / `COVERAGE_TOMBSTONE_CORE_HASH`;
- Source Coverage Statement: `SOURCE_COVERAGE_ATTESTATION` / `COVERAGE_STATEMENT_CORE_HASH`; and
- Source Witness Attestation: `SOURCE_WITNESS_ATTESTATION` / `SOURCE_WITNESS_ATTESTATION_CORE_HASH`.

For all three carriers the core is canonicalized independently, the core digest is recomputed before proof verification, proof objects remain outside the core and proof validity never substitutes for signer identity, authority, witness independence, time, completeness or substantive truth.
...
The carrier additionally contains the registered `tombstoneCoreHash` and one or more embedded `tombstoneProofs`. Every proof MUST be a complete Signature Proof v0.2 object using `COVERAGE_TOMBSTONE_ATTESTATION` / `COVERAGE_TOMBSTONE_CORE_HASH`, and its `signedObject.digest` MUST equal the independently recomputed `tombstoneCoreHash`. The hash is:

```text
tombstoneCoreHash = SHA-256(UTF8(RFC8785-JCS(tombstoneCore)))
```

`tombstoneProofs` are not members of `tombstoneCore`; they are ordered by ascending `proofId` Unicode code point and duplicate `proofId` values are invalid. The corresponding coverage leaf's `tombstoneCommitment` MUST equal `tombstoneCoreHash`.
...
The event-set tree uses the exact domain-separated SHA-256 tree construction of `DDT-TIME-CHECKPOINT-PROFILE-0.2-DRAFT.1`:
...
The statement carrier contains `statementCore`, the registered `statementCoreHash` and one or more embedded `statementProofs`:

```text
statementCoreHash = SHA-256(UTF8(RFC8785-JCS(statementCore)))
```

Every `statementProofs` member MUST be a complete Signature Proof v0.2 object using `SOURCE_COVERAGE_ATTESTATION` / `COVERAGE_STATEMENT_CORE_HASH`; its `signedObject.digest` MUST equal the independently recomputed `statementCoreHash`. Proofs are ordered by ascending `proofId` Unicode code point and duplicate `proofId` values are invalid. The proof objects and later witness attestations are not inserted into `statementCore`. This avoids circular commitment construction.
...
A witness attestation uses a non-circular carrier. Its `attestationCore` contains domain `DDT_SOURCE_WITNESS_ATTESTATION_V1`, witness and statement identity, observation scope, observed roots/commitments, independence evidence references and optional witnessed-time claim. The carrier separately contains `attestationCoreHash`, one or more embedded `attestationProofs` and external-time evidence references. Every proof MUST be a complete Signature Proof v0.2 object using `SOURCE_WITNESS_ATTESTATION` / `SOURCE_WITNESS_ATTESTATION_CORE_HASH`; its `signedObject.digest` MUST equal the independently recomputed `attestationCoreHash`. Proofs are ordered by ascending `proofId` Unicode code point and duplicate `proofId` values are invalid.
...
8. Source Coverage Statement core-hash equality, exact Signature Proof v0.2 purpose/target/digest binding, signer identity and authority;
...
- Source Coverage Statement bytes and complete Signature Proof v0.2 objects;
- source-state snapshot bytes or exact independently retrievable commitments and procedures;
- witness statements, complete Signature Proof v0.2 objects, identity/authority/independence evidence and proof profiles;
...
## 22. v0.2 common-proof validation requirements

For every v0.2 tombstone, coverage statement and witness attestation, a conforming verifier MUST:

1. validate the carrier against the v0.2 evidence schema;
2. canonicalize the relevant core with RFC 8785 JCS and recompute its SHA-256 digest;
3. require equality with the carrier's registered core-hash field;
4. reject an empty proof set, duplicate `proofId`, non-deterministic proof order or a legacy `proofRefs` replacement;
5. validate every proof against the exact Signature Proof v0.2 schema and Ed25519/JCS v0.2 profile selected by the historical artifacts;
6. require the exact purpose/target pair for that carrier and equality between `signedObject.digest` and the recomputed core hash; and
7. evaluate signature validity separately from identity, authority, witness independence, independent time and scoped completeness.

A proof from one coverage carrier MUST NOT be replayed into either of the other two carrier classes even when the underlying digest bytes happen to be equal.
