# DDT Cryptographic Renewal Event v0.2 — Draft

**Project:** Diamond Data Chain (DDC)  
**Specification ID:** `DDT-CRYPTOGRAPHIC-RENEWAL-EVENT-0.2-DRAFT.1`  
**Version:** `0.2.0-draft.1`  
**Status:** DRAFT — Signature Proof v0.2 integrated; aligned signed fixture, schema-engine and interoperability tests pending  
**Date:** 2026-09-05  
...
- `schemas/ddt-cryptographic-renewal-event-v0.2.schema.json`;
- `schemas/ddt-cryptographic-renewal-profile-v0.2.schema.json`; and
- `profiles/ddt-baseline-cryptographic-renewal-profile-v0.2.json`.

The bundled baseline is `TEST_ONLY`. Version 0.2 directly integrates the common Signature Proof v0.2 carrier binding for renewal attestations; candidate status still requires aligned signed/tampered fixtures, schema-engine validation and independent reproduction.
...
`renewalProofs` contains complete Signature Proof v0.2 objects outside the hashed core. Each renewal proof MUST use the exact registered `CRYPTOGRAPHIC_RENEWAL_ATTESTATION` / `RENEWAL_CORE_HASH` pair and its `signedObject.digest` MUST equal the independently recomputed `renewalCoreHash`. Proofs MUST be ordered by ascending Unicode code-point order of `proofId`, and duplicate proof IDs MUST be rejected.

Changing a renewal proof does not change `renewalCoreHash`, but every proof-core field except `proofValue` participates in the Signature Proof v0.2 signing input; proof metadata tampering therefore invalidates that proof. Every independent time proof MUST bind the exact core hash directly or through a reproducible aggregate inclusion path.
...
Version 0.2 directly references `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`. Every `renewalProofs` entry MUST be a complete common Signature Proof v0.2 object with exactly:

```text
proofPurpose       = CRYPTOGRAPHIC_RENEWAL_ATTESTATION
signedObject.type  = RENEWAL_CORE_HASH
signedObject.digest = renewalCoreHash
```

The verifier MUST independently recompute `renewalCoreHash` before accepting the carrier binding. A valid signature over another digest, another purpose or another signed-object type is not a valid renewal proof. Signature validity, verification-method integrity, witness identity, witness authority, algorithm lifecycle and independent time remain separate verification axes.

The event remains DRAFT until full signature, schema-engine, metadata-tamper and independent-reproduction fixtures pass.
...
16. renewal proof wrong purpose;
17. renewal proof wrong signed-object type;
18. renewal proof signed-object digest mismatch;
19. duplicate renewal proof ID;
20. unauthorized renewing witness;
21. asserted time without external time proof;
22. time proof bound to another core;
23. failed-attempt event preserved without continuity claim;
24. preservation-only migration;
25. second renewal chaining from the first;
26. two conflicting renewals preserved;
27. substantive-truth overclaim rejected; and
28. complete offline reproduction with DDC services disabled.

## 20. Candidate and final gates

Candidate status requires the event/profile schemas, exact profile artifact, Signature Proof v0.2 and Verification Result v0.2 compatibility, reference verifier and all mandatory fixtures. The common proof binding is integrated at draft/schema level but is not production evidence until those fixtures are complete.
