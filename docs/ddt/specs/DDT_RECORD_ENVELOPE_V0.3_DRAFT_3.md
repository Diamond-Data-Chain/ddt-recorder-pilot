...
**Specification ID:** `DDT-ENVELOPE-0.3-DRAFT.3`  
...
This revision retains the exact companion-artifact boundaries introduced by Draft.2 and replaces the envelope's divergent local registration-signature shape with the common `DDT-SIGNATURE-PROOF-0.2-DRAFT.1` object. It also selects `DDT-VERIFICATION-RESULT-0.2-DRAFT.1` and its exact axis/reason registries as the Draft.3 verification-output baseline. It remains a draft until reference implementations, full schema-engine fixtures and independent interoperability checks pass.
...
17. Verification output for Draft.3 testing MUST conform to `DDT-VERIFICATION-RESULT-0.2-DRAFT.1` and the exact axis/reason registries bound by that result, unless an exact compatible successor is selected by a later contract and envelope version.
18. Registration signature proofs MUST conform to `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`; Draft.3 does not define a second local proof object.
...
Both fields are REQUIRED and fixed for Draft.3.
...
### 6.1 Draft.3 companion baseline

The Draft.3 envelope schema directly resolves these exact structural dependencies:

- `DDT-EVIDENCE-MANIFEST-0.1-DRAFT.1`;
- `DDT-PRESERVATION-CONTRACT-0.1-DRAFT.1`; and
- `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`.

The historical Preservation Contract, Conformance Receipt, registration mechanism and evidence set MUST additionally bind every applicable companion artifact used to interpret or verify the record. For Draft.3 testing this includes, where applicable, the active Identifier, Enterprise, Relationship, Identity/Authority, Time/Checkpoint, Event Coverage, Network Finality, Offline Package and Cryptographic Renewal profiles.

The Draft.3 verification-output baseline is `DDT-VERIFICATION-RESULT-0.2-DRAFT.1`, including its exact machine-readable axis and reason-code registries. Those later-produced results are separate append-only artifacts and are not members of the historical envelope.
...
The complete object MUST conform to the exact Evidence Manifest specification and schema selected by the historical Preservation Contract. For Draft.3 testing, the required baseline is `DDT-EVIDENCE-MANIFEST-0.1-DRAFT.1`.
...
The complete object MUST conform to the exact Preservation Contract specification and schema applicable to Draft.3. The baseline is `DDT-PRESERVATION-CONTRACT-0.1-DRAFT.1`.
...
For Draft.3, the complete receipt MUST conform to `DDT-CONFORMANCE-RECEIPT-0.1-DRAFT.1`. The envelope does not define a second or reduced receipt structure.
...
The `proofProfile` MUST digest-bind the exact Signature Profile used. For the Draft.3 baseline this is the `TEST_ONLY` `ddt.signature.ed25519-jcs-v2` profile. A later production profile requires its own exact version, digest and governed status.
...
The bound proof profile MUST define the exact signing input, including domain separation and encoding. Under the Draft.3 baseline, the profile uses the `DDT_SIGNATURE_PROOF_V2` domain wrapper and pure RFC 8032 Ed25519. A verifier MUST NOT guess how a signature input was constructed or reinterpret a v0.1 proof under v0.2.
...
Because this document is a draft, such a claim MUST be expressed as `implements DDT-ENVELOPE-0.3-DRAFT.3 for testing` and MUST NOT be expressed as final production conformance.
...
Draft.3 integrates the current Preservation Contract, Enterprise Profile, Evidence Manifest and Conformance Receipt boundaries, the common Signature Proof v0.2 registration proof and the Verification Result v0.2 output model. These artifacts remain non-final until their required fixtures pass.
...
**End of DDT Record Envelope v0.3 Draft.3 (`0.3.0-draft.3`)**
