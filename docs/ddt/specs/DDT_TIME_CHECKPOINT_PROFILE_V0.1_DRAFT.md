# DDT Time, Ordering and Checkpoint Profile v0.1 Draft.1

**Registry ID:** `DDT-TIME-CHECKPOINT-PROFILE-0.1-DRAFT.1`  
**Profile artifact ID:** `ddt.time-checkpoint.sha256-jcs-v1`  
...
- `schemas/ddt-time-checkpoint-evidence-v0.1.schema.json`;
- `schemas/ddt-time-checkpoint-profile-v0.1.schema.json`; and
- `profiles/ddt-time-checkpoint-profile-v0.1.json`.
...
### 9.2 Checkpoint proof

Every checkpoint MUST have at least one valid integrity/authorship proof under an exact proof profile.

The next versioned Signature Proof integration revision MUST add an exact checkpoint proof purpose and `CHECKPOINT_CORE_HASH` signed-object type. Until that integration and its fixtures exist, this checkpoint profile remains draft-only.
...
- add exact checkpoint proof-purpose and signed-object bindings to the Signature Proof/Profile;
...
- signature and Verification Result integration revisions;
