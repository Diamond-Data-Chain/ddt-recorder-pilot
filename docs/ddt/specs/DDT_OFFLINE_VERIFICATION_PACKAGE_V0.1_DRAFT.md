# DDT Offline Verification Package v0.1 — Draft

**Project:** Diamond Data Chain (DDC)  
**Specification ID:** `DDT-OFFLINE-VERIFICATION-PACKAGE-0.1-DRAFT.1`  
**Version:** `0.1.0-draft.1`  
**Status:** DRAFT — reference packager/verifier and complete fixtures pending  
**Date:** 2026-09-04  
...
3. any detached package-manifest proofs referenced outside `manifestCore`;
...
3. create detached proof artifacts that sign the exact `manifestCoreHash` under a bound proof profile; and
4. place `manifestCore`, `manifestCoreHash` and the detached proof Artifact References in the carrier.

The manifest carrier itself is not listed in `manifestCore.files`. Detached manifest-proof artifacts are also outside `manifestCore.files`; their exact digests and locations are bound by `manifestProofs` in the outer carrier. This prevents a signature/file-digest cycle.

Changing any core field or listed file changes `manifestCoreHash`. Changing a detached proof does not change the core hash, but its Artifact Reference digest and proof verification fail.
...
- detached manifest-proof verification;
...
6. verify each detached manifest-proof artifact and its digest;
...
A production package MUST carry at least one detached manifest proof under an exact signature profile. Proof validity, verification-method integrity, signer identity, signer authority and proof time remain separate axes.
...
Draft.1 depends on a compatible Signature Proof successor that registers `OFFLINE_PACKAGE_ATTESTATION` and `MANIFEST_CORE_HASH`. Until that integration is implemented and tested, the package format remains DRAFT and no production package-conformance claim is permitted.
...
10. invalid detached manifest proof;
...
- integration with the registered Signature Proof and Verification Result versions; and
