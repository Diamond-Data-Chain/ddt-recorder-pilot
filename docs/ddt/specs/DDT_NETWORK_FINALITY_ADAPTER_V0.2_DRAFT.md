# DDT Network Finality Adapter v0.2 — Draft

**Project:** Diamond Data Chain (DDC)  
**Specification ID:** `DDT-NETWORK-FINALITY-ADAPTER-0.2-DRAFT.1`  
...
**Common signature-proof dependency:** `DDT-SIGNATURE-PROOF-SCHEMA-0.2-DRAFT.1`  
**Verification-result dependency:** `DDT-VERIFICATION-RESULT-SCHEMA-0.2-DRAFT.1`

## v0.2 successor boundary

This successor intentionally preserves the v0.1 deterministic test-ledger target extraction, transaction identity, inclusion-tree, block identity, finality-core, historical trust-state, fork/conflict and time-separation semantics. The normative carrier change is limited to replacing detached finality `proofRefs` with complete common Signature Proof v0.2 objects bound to the finalized `finalityCoreHash`. The bundled deterministic profile remains `TEST_ONLY`; this document does not claim a production DDC consensus or finality profile.
...
- `schemas/ddt-network-finality-profile-v0.2.schema.json`;
- `schemas/ddt-network-finality-evidence-v0.2.schema.json`; and
- `profiles/ddt-deterministic-finality-test-profile-v0.2.json`.
...
The v0.2 finality certificate carrier contains the core, registered core hash and complete common Signature Proof v0.2 objects in `finalityProofs`. Detached `proofRefs` are not used by the v0.2 carrier. Every finality proof MUST use `NETWORK_FINALITY_ATTESTATION` / `FINALITY_CORE_HASH`, and its signed-object digest MUST equal the independently recomputed `finalityCoreHash`. Proofs remain outside `finalityCore`, MUST be ordered by ascending `proofId` Unicode code point, and duplicate proof IDs MUST be rejected. A verifier independently recomputes the core hash before verifying proof cryptography, signer identity/lifecycle, finality authority and quorum.
...
`DDT-NETWORK-FINALITY-EVIDENCE-0.2-DRAFT.1` contains:
...
Verification Result v0.2 provides the network/finality result axes required by this adapter, including:
...
- complete Signature Proof v0.2 objects using the Ed25519/JCS v0.2 test profile;
