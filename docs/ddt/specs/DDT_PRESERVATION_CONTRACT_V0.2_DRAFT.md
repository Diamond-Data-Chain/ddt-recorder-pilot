# DDT Preservation Contract v0.2 — Draft

**Project:** Diamond Data Chain (DDC)  
**Specification ID:** `DDT-PRESERVATION-CONTRACT-0.2-DRAFT.1`  
**Version:** `0.2.0-draft.1`  
**Status:** DRAFT — not approved for production conformance claims  
**Date:** 2026-09-04  
**Parent standard:** `DDC-TRS-2.0-DRAFT.1`
**Supersedes for new test artifacts:** `DDT-PRESERVATION-CONTRACT-0.1-DRAFT.1`

## 1. Purpose

A DDT Preservation Contract identifies the exact rules and artifacts under which one DDT Record was formed and validated.

Version 0.2 closes the broad `cryptographicProfile` ambiguity, binds the exact identifier, registration-signature, conformance, verification-result and registry selection points, and materializes whether six optional preservation capabilities were required, conditional or outside the contract at registration time.
...
A v0.1 Preservation Contract remains a v0.1 artifact. It MUST NOT be relabeled as v0.2 or interpreted as having capability declarations or artifact roles that its original bytes did not contain.
...
The successor to `DDT-ENVELOPE-0.3-DRAFT.3` MUST:

1. bind the complete v0.2 Preservation Contract structure without a local reduced duplicate;
2. use its exact core artifact roles and capability declarations;
3. include the resulting complete contract commitment in `recordHash` under the existing non-circular construction;
4. require the Conformance Receipt to report materialized capability/rule coverage; and
5. preserve every limiting capability result separately in Verification Result v0.2.

This appendix records the required integration boundary and prevents Draft.3 from being mistaken for a freeze candidate merely because its current v0.1 contract dependency remains structurally valid.

---

**End of DDT Preservation Contract v0.2 Draft (`0.2.0-draft.1`)**
