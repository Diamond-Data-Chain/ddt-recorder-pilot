...
**Specification ID:** `DDT-ENVELOPE-0.3-DRAFT.4`  
...
This revision retains the exact companion-artifact boundaries introduced by Draft.2 and the common Signature Proof v0.2 and Verification Result v0.2 integrations completed in Draft.3. Draft.4 replaces the earlier contract/receipt baselines with `DDT-PRESERVATION-CONTRACT-0.2-DRAFT.1` and `DDT-CONFORMANCE-RECEIPT-0.2-DRAFT.1`. The registration statement binds the complete v0.2 receipt commitment while the receipt remains outside `recordHash`, preserving the non-circular construction. It remains a draft until reference implementations, full schema-engine fixtures and independent interoperability checks pass.
...
14. The embedded Preservation Contract MUST conform to `DDT-PRESERVATION-CONTRACT-0.2-DRAFT.1`.
...
16. The registration statement binds the complete Conformance Receipt commitment defined by `DDT-CONFORMANCE-RECEIPT-0.2-DRAFT.1`.
17. Verification output for Draft.4 testing MUST conform to `DDT-VERIFICATION-RESULT-0.2-DRAFT.1` and the exact axis/reason registries bound by that result, unless an exact compatible successor is selected by a later contract and envelope version.
18. Registration signature proofs MUST conform to `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`; Draft.4 does not define a second local proof object.
...
Both fields are REQUIRED and fixed for Draft.4.
...
### 6.1 Draft.4 companion baseline

The Draft.4 envelope schema directly resolves these exact structural dependencies:

- `DDT-EVIDENCE-MANIFEST-0.1-DRAFT.1`;
- `DDT-PRESERVATION-CONTRACT-0.2-DRAFT.1`; and
- `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`.

The historical Preservation Contract v0.2, Conformance Receipt v0.2, registration mechanism and evidence set MUST additionally bind every applicable companion artifact used to interpret or verify the record. For Draft.4 testing this includes, where applicable, the active Identifier, Enterprise, Relationship, Identity/Authority, Time/Checkpoint, Event Coverage, Network Finality, Offline Package and Cryptographic Renewal profiles. All thirteen contract artifact roles and all six capability classes MUST be represented exactly as required by those companion specifications.

The Draft.4 verification-output baseline is `DDT-VERIFICATION-RESULT-0.2-DRAFT.1`, including its exact machine-readable axis and reason-code registries. Those later-produced results are separate append-only artifacts and are not members of the historical envelope.
...
The complete object MUST conform to the exact Evidence Manifest specification and schema selected by the historical Preservation Contract. For Draft.4 testing, the required baseline is `DDT-EVIDENCE-MANIFEST-0.1-DRAFT.1`.
...
The complete object MUST conform to the exact Preservation Contract specification and schema applicable to Draft.4. The required baseline is `DDT-PRESERVATION-CONTRACT-0.2-DRAFT.1`. The contract MUST materialize all thirteen core artifact roles and all six capability classes as `REQUIRED`, `CONDITIONAL` or `NOT_APPLICABLE`; an omitted role or capability is not equivalent to `NOT_APPLICABLE`.
...
For Draft.4, the complete receipt MUST conform to `DDT-CONFORMANCE-RECEIPT-0.2-DRAFT.1`. The envelope does not define a second or reduced receipt structure. Its receipt core MUST bind the same `recordHash` and `preservationContractCommitment` carried by this envelope, account for the contract's closed artifact and capability sets, and use the v0.2 deterministic overall-result derivation.
...
Instead, `conformanceReceiptCommitment` is computed exactly as defined by Conformance Receipt v0.2 over its `specification`, `receiptCore`, `receiptCoreHash` and `proofs`, after sorting proofs by `proofId` and rejecting duplicate proof identifiers. That commitment is included in the registration statement. At least one receipt proof MUST use Signature Proof v0.2 with `proofPurpose = CONFORMANCE_ATTESTATION` and `signedObject.type = RECEIPT_CORE_HASH`.
...
The `proofProfile` MUST digest-bind the exact Signature Profile used. For the Draft.4 baseline this is the `TEST_ONLY` `ddt.signature.ed25519-jcs-v2` profile. A later production profile requires its own exact version, digest and governed status.
...
The bound proof profile MUST define the exact signing input, including domain separation and encoding. Under the Draft.4 baseline, the profile uses the `DDT_SIGNATURE_PROOF_V2` domain wrapper and pure RFC 8032 Ed25519. A verifier MUST NOT guess how a signature input was constructed or reinterpret a v0.1 proof under v0.2.
...
Because this document is a draft, such a claim MUST be expressed as `implements DDT-ENVELOPE-0.3-DRAFT.4 for testing` and MUST NOT be expressed as final production conformance.
...
Draft.4 directly integrates Preservation Contract v0.2 and the complete Conformance Receipt v0.2 commitment, while retaining the Enterprise Profile, Evidence Manifest, common Signature Proof v0.2 registration proof and Verification Result v0.2 output boundaries. These artifacts remain non-final until their required fixtures pass.

The following work is still required before freeze:

1. a Preservation Contract successor that explicitly and reproducibly binds every applicable active companion profile and registry;
2. immutable artifact publication and transitive dependency-resolution policy;
3. production canonicalization, commitment, registration-signature and proof profiles;
4. migration of the remaining signature-bearing carrier drafts to Signature Proof v0.2;
5. append-only proof-augmentation structure and migration/backward-verification procedure;
6. production network-finality profile after the DDC mainnet protocol and consensus rules are frozen;
7. reference producer, registration validator, offline packager and independent verifier;
8. positive, tampered, historical, schema-engine, cross-language and cross-implementation fixtures;
9. domain profiles and examples for AI, manufacturing and transport without changing the core envelope; and
10. disposition of all remaining review findings before candidate freeze.

---

**End of DDT Record Envelope v0.3 Draft.4 (`0.3.0-draft.4`)**
