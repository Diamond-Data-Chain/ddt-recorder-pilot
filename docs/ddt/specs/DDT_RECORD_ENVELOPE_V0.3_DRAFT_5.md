...
**Specification ID:** `DDT-ENVELOPE-0.3-DRAFT.5`  
...
This revision retains the non-circular commitment and proof boundaries established through Drafts.2–4 and completes the current companion-artifact integration baseline. Draft.5 directly uses `DDT-PRESERVATION-CONTRACT-0.3-DRAFT.1`, whose contract commitment includes the exact deterministic transitive normative dependency closure, and `DDT-CONFORMANCE-RECEIPT-0.3-DRAFT.1`, whose receipt core separately evaluates that exact closure. The registration statement binds the complete v0.3 receipt commitment while the receipt remains outside `recordHash`; no new record-hash field is introduced solely for the dependency closure because the closure is already protected by `preservationContractCommitment`. The common Signature Proof v0.2 carrier family is now integrated at draft/schema level for registration, conformance, verification results, checkpoints, event coverage, network finality, offline packages and cryptographic renewal. This remains a draft until the reference implementations, full positive/tampered/historical/schema-engine fixtures and independent interoperability checks pass.
...
10. Every registration signature uses the common Signature Proof v0.2 structure and exact `DDT_REGISTRATION` / `REGISTRATION_STATEMENT_HASH` pair.
...
14. The embedded Preservation Contract MUST conform to `DDT-PRESERVATION-CONTRACT-0.3-DRAFT.1`.
...
16. The registration statement binds the complete Conformance Receipt commitment defined by `DDT-CONFORMANCE-RECEIPT-0.3-DRAFT.1`.
17. Verification output for Draft.5 testing MUST conform to `DDT-VERIFICATION-RESULT-0.2-DRAFT.1` and the exact axis/reason registries bound by that result, unless an exact compatible successor is selected by a later contract and envelope version.
18. Registration signature proofs MUST conform to `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`; Draft.5 does not define a second local proof object.
19. At least one registration signature proof is REQUIRED. Additional registration proofs remain permitted when the Preservation Contract requires multiple signers.
20. Proof validity, historical key identity, signer authority, independent time, network finality, event coverage, semantic reconstruction and substantive truth remain separate verification axes.
21. Preservation Contract v0.3 binds the exact deterministic transitive normative dependency closure inside the committed contract bytes; Draft.5 does not duplicate that closure as a second envelope field.
22. Conformance Receipt v0.3 separately evaluates the same exact dependency closure and binds its closure digest and evaluated-dependency-set hash before the complete receipt commitment is registered.
23. The Signature Proof v0.2 carrier family for checkpoints, event coverage, network finality, offline packages and cryptographic renewal remains outside the historical `recordHash` except where the exact Preservation Contract commits the governing profile dependencies.
...
Both fields are REQUIRED and fixed for Draft.5.
...
If that schema resolves another schema or profile through `$ref` or an equivalent mechanism, every transitive normative dependency actually used MUST appear by exact Artifact Reference in `preservationContract.normativeDependencies.artifacts`, MUST be covered one-for-one by Conformance Receipt v0.3 `normativeDependencyEvaluations`, and MUST be preserved by any conforming Offline Verification Package when the declared verification plan requires it. Direct Preservation Contract roles remain separately represented by the receipt `artifactEvaluations`; the transitive closure does not replace those direct-role evaluations. A verifier MUST NOT resolve an unversioned or changed current resource as a historical dependency. Missing, extra, reordered, substituted or digest-mismatched historical dependency bytes make complete historical conformance reproduction unavailable or unresolved; they do not authorize live substitution.

### 6.1 Draft.5 companion baseline

The Draft.5 envelope schema directly resolves these exact structural dependencies:

- `DDT-EVIDENCE-MANIFEST-0.1-DRAFT.1`;
- `DDT-PRESERVATION-CONTRACT-0.3-DRAFT.1`; and
- `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`.

For the current Draft.5 integration baseline, the Preservation Contract v0.3 dependency closure and applicable capability selections bind the exact active companion artifacts needed for historical reproduction, including where applicable:

- `DDT-IDENTIFIER-PROFILE-0.1-DRAFT.1`;
- `DDT-ENTERPRISE-PROFILE-0.1-DRAFT.1`;
- `DDT-RELATIONSHIP-VOCABULARY-0.1-DRAFT.1`;
- `DDT-IDENTITY-AUTHORITY-PROFILE-0.1-DRAFT.1`;
- `DDT-TIME-CHECKPOINT-PROFILE-0.2-DRAFT.1`;
- `DDT-EVENT-COVERAGE-PROFILE-0.2-DRAFT.1`;
- `DDT-NETWORK-FINALITY-ADAPTER-0.2-DRAFT.1` together with its explicitly `TEST_ONLY` deterministic profile where used for testing;
- `DDT-OFFLINE-VERIFICATION-PACKAGE-0.2-DRAFT.1`;
- `DDT-CRYPTOGRAPHIC-RENEWAL-EVENT-0.2-DRAFT.1`;
- `DDT-SIGNATURE-PROOF-0.2-DRAFT.1`; and
- `DDT-VERIFICATION-RESULT-0.2-DRAFT.1` with its exact axis and reason-code registries.

The exact historical set is determined by the concrete Preservation Contract v0.3, not by this informative baseline list. All thirteen direct contract artifact roles and all six capability classes MUST be materialized exactly, and the complete transitive normative dependency closure MUST be deterministic, digest-bound and receipt-evaluated. A `NOT_APPLICABLE` capability does not enter the historical closure merely because a later implementation supports it.

Verification Result v0.2 remains a separate append-only output. Time checkpoints, coverage evidence, network/finality evidence, offline packages and cryptographic-renewal events likewise remain separate companion artifacts and MUST NOT be retroactively inserted into `recordHash` unless a future version explicitly changes that commitment boundary.
...
The complete object MUST conform to the exact Evidence Manifest specification and schema selected by the historical Preservation Contract. For Draft.5 testing, the required baseline is `DDT-EVIDENCE-MANIFEST-0.1-DRAFT.1`.
...
The complete object MUST conform to the exact Preservation Contract specification and schema applicable to Draft.5. The required baseline is `DDT-PRESERVATION-CONTRACT-0.3-DRAFT.1`. The contract MUST materialize all thirteen direct artifact roles and all six capability classes as `REQUIRED`, `CONDITIONAL` or `NOT_APPLICABLE`, and MUST carry the exact deterministic `normativeDependencies` closure and `closureDigest`. An omitted role or capability is not equivalent to `NOT_APPLICABLE`, and a live-resolved dependency is not equivalent to a historically bound closure member.
...
All thirteen direct artifact references defined by Preservation Contract v0.3 are REQUIRED. The contract MUST additionally bind the exact transitive normative dependency closure needed to parse, validate and verify those direct selections and every applicable capability profile.
...
`normativeDependencies.artifacts` MUST be canonically ordered exactly as required by Preservation Contract v0.3, contain no duplicate `(artifactId, version)` pair and reproduce `normativeDependencies.closureDigest`. The Envelope does not duplicate this closure in a second top-level field; it is already included in the canonical Preservation Contract bytes and therefore in `preservationContractCommitment` and `recordHash`.
...
Changing specification identity, any direct artifact digest/version/reference, any normative-dependency member or closure digest, any capability branch/profile, validator value, applicability field or materialized obligation MUST change `preservationContractCommitment` and therefore `recordHash`.
...
For Draft.5, the complete receipt MUST conform to `DDT-CONFORMANCE-RECEIPT-0.3-DRAFT.1`. The envelope does not define a second or reduced receipt structure. Its receipt core MUST bind the same `recordHash` and `preservationContractCommitment` carried by this envelope, bind the exact `preservationContract.normativeDependencies.closureDigest`, account separately for all thirteen direct artifact roles, every exact normative dependency and all six capability classes, and use the v0.3 deterministic overall-result derivation.
...
- the complete direct-artifact evaluation-set hash;
- the complete normative-dependency evaluation-set hash;
...
Instead, `conformanceReceiptCommitment` is computed exactly as defined by Conformance Receipt v0.3 over its `specification`, `receiptCore`, `receiptCoreHash` and `proofs`, after canonical ordering of the receipt's direct artifact, normative dependency and evidence evaluation sets as required by that specification, sorting proofs by `proofId` and rejecting duplicate proof identifiers. That commitment is included in the registration statement. At least one receipt proof MUST use Signature Proof v0.2 with `proofPurpose = CONFORMANCE_ATTESTATION` and `signedObject.type = RECEIPT_CORE_HASH`.
...
The receipt MUST preserve the exact validation input commitment, complete direct-artifact, normative-dependency and evaluated-evidence sets, closure digest, per-rule coverage, materialized-obligation origins, deterministic overall-result derivation and receipt proof scope required by its companion specification.
  ...
  "signatureProofs": [
    {
      ...
      "proofValue": "...",
      "createdAtClaim": "2026-09-04T08:31:00Z"
    }
  ],
  "timeProofs": []
}
...
### 15.1 Common structure

Every `signatureProofs` entry MUST conform to the exact `DDT-SIGNATURE-PROOF-0.2-DRAFT.1` schema. The envelope does not reproduce or subset that proof shape.

A registration proof is:
...
The `proofProfile` MUST digest-bind the exact Signature Profile used. For the Draft.5 baseline this is the `TEST_ONLY` `ddt.signature.ed25519-jcs-v2` profile. A later production profile requires its own exact version, digest and governed status.

### 15.2 Required proof set and binding

At least one registration signature proof is REQUIRED. Each proof MUST include every field required by Signature Proof v0.2, including:
...
The only permitted purpose/target pair in the envelope's `registration.signatureProofs` array is:

```text
DDT_REGISTRATION -> REGISTRATION_STATEMENT_HASH
```

Other valid Signature Proof v0.2 purposes belong to their corresponding carrier artifacts and MUST be rejected from this array.

### 15.3 Signed object and signing input
...
The bound proof profile MUST define the exact signing input, including domain separation and encoding. Under the Draft.5 baseline, the profile uses the `DDT_SIGNATURE_PROOF_V2` domain wrapper and pure RFC 8032 Ed25519. A verifier MUST NOT guess how a signature input was constructed or reinterpret a v0.1 proof under v0.2.
...
Signature Proof conformance and mathematical signature validity do not by themselves establish historical verification-method resolution, identity, authority or independent time. A verifier MUST emit those results separately using `DDT-VERIFICATION-RESULT-0.2-DRAFT.1`.
...
15. validate every registration proof against Signature Proof v0.2, enforce the exact registration purpose/target pair, compare the signed digest to `registrationStatementHash`, and produce or verify the contract-required signature set;
...
13. emit axis-specific statuses, reason codes, evaluated artifact digests and verifier metadata using Verification Result v0.2 and its exact bound Axis and Reason-Code Registries.
...
- independently reconstruct and compare the exact Preservation Contract v0.3 normative dependency closure and closure digest;
- require Conformance Receipt v0.3 to cover every direct role, every exact normative dependency and every capability branch selected by the contract;
- use the exact Conformance Receipt commitment and proof scope;
- use the common Signature Proof v0.2 object and reject every non-registration or mismatched purpose/target pair in `registration.signatureProofs`;
- require at least one registration signature proof;
- emit multi-axis verification results under the exact contract-bound Verification Result v0.2 profile and registries; and
- pass the normative positive and tampered fixtures.

Because this document is a draft, such a claim MUST be expressed as `implements DDT-ENVELOPE-0.3-DRAFT.5 for testing` and MUST NOT be expressed as final production conformance.
...
5. Preservation Contract direct artifact/version/capability tamper;
6. missing, extra, reordered, substituted and digest-mismatched normative dependency closure members;
7. Conformance Receipt normative-dependency evaluation omission/substitution and closure-digest mismatch;
8. identity and subject-namespace tamper;
9. predecessor substitution and missing predecessor;
10. target identity/hash/relationship-type tamper;
11. unsorted and duplicate collection rejection;
12. Conformance Receipt substitution;
13. registration statement substitution;
14. registration signature tamper, wrong purpose/target and empty proof set;
15. unresolved key identity and revoked-key historical-state cases;
16. missing and invalid external time proof;
17. required missing evidence versus non-required missing context;
18. historical profile v1.0 verification after profile v1.1 publication;
19. offline verification with all DDC-operated services unavailable and live dependency resolution denied;
20. cross-language RFC 8785 commitment reproduction;
21. `CASE-HTB-01`, `CASE-HSR-01A`, `CASE-HSR-01B`, `CASE-PCR-01`; and
22. a v0.3-aligned cryptographic-renewal case derived from `CASE-CRR-01`.
...
Draft.5 completes the current envelope-layer integration of Preservation Contract v0.3, Conformance Receipt v0.3, Signature Proof v0.2, Verification Result v0.2 and the completed v0.2 checkpoint/coverage/finality/offline/renewal carrier family. The deterministic normative dependency closure is protected through `preservationContractCommitment`; the receipt independently evaluates that same closure; and the complete receipt commitment remains post-`recordHash` and is bound by the registration statement. No historical Draft.4 or earlier bytes are rewritten.

The following work is still required before candidate freeze:

1. immutable publication/archival rules for exact schemas, profiles, registries, validators and historical dependency bytes;
2. production canonicalization, structural-commitment, registration-signature and proof profiles to replace `TEST_ONLY` bootstrap profiles where applicable;
3. append-only proof-augmentation and backward-verification procedure for historical/frozen envelopes and companion artifacts;
4. production Network Finality profile after the DDC mainnet protocol and consensus rules are frozen;
5. reference producer, registration validator, offline packager and independent verifier;
6. complete positive, tampered, historical-reproduction, schema-engine, malicious-package, cross-language and cross-implementation fixtures;
7. Enterprise Profile examples and fixtures for AI, manufacturing and transport without changing the core envelope;
8. production cryptographic lifecycle/renewal-trigger governance and algorithm-deprecation fixtures; and
9. disposition of all remaining external-review and traceability findings before candidate freeze.

The envelope format integration itself is no longer an open architectural item. Remaining work is implementation, executable verification evidence, production-profile governance and release discipline.

---

**End of DDT Record Envelope v0.3 Draft.5 (`0.3.0-draft.5`)**
